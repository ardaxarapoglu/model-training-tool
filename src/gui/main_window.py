...
class MainWindow(QMainWindow):
    ...
    def _setup_menu(self):
        ...
        act = QAction("Save Split…", self)
        act.setToolTip(
            "Save the current train/validation/test assignment of every time frame "
            "to a JSON file so it can be reloaded later or referenced in a report."
        )
        act.triggered.connect(self._save_split)
        tools_m.addAction(act)

        act = QAction("Load Split…", self)
        act.setToolTip(
            "Apply a previously saved split to the current experiments.\n"
            "Accepts a split file (saved via 'Save Split') or a result JSON "
            "(the split is stored there automatically after each training run)."
        )
        act.triggered.connect(self._load_split)
        tools_m.addAction(act)
    ...
    def _save_split(self):
        from ..utils.split_manager import save_split
        experiments = self.exp_panel.get_config()
        if not experiments:
            QMessageBox.warning(self, "No Experiments", "Define experiments before saving a split.")
            return
        path, _ = QFileDialog.getSaveFileName(
            self, "Save Split", "split.json", "JSON (*.json)"
        )
        if not path:
            return
        if not path.endswith(".json"):
            path += ".json"
        try:
            save_split(experiments, path)
            self.lbl_status.setText(f"Split saved: {path}")
            QMessageBox.information(
                self, "Split Saved",
                f"Split configuration written to:\n{path}\n\n"
                "You can reload it later via  Tools → Load Split,\n"
                "or open a result JSON directly (the split is stored there too)."
            )
        except Exception as exc:
            QMessageBox.critical(self, "Error", f"Could not save split:\n{exc}")

    def _load_split(self):
        from ..utils.split_manager import load_split_file, apply_split
        path, _ = QFileDialog.getOpenFileName(
            self, "Load Split", "", "JSON (*.json)"
        )
        if not path:
            return
        try:
            split_file = load_split_file(path)
            experiments = self.exp_panel.get_config()
            updated = apply_split(experiments, split_file["splits"])
            self.exp_panel.set_config(updated)
            name    = split_file.get("name", "")
            created = split_file.get("created", "")
            label   = f"'{name}'" if name else os.path.basename(path)
            suffix  = f"  (created {created})" if created else ""
            self.lbl_status.setText(f"Split loaded: {label}{suffix}")
            QMessageBox.information(
                self, "Split Loaded",
                f"Split {label}{suffix} applied to experiments.\n"
                "Review the Experiments tab to confirm."
            )
        except Exception as exc:
            QMessageBox.critical(self, "Error", f"Could not load split:\n{exc}")
